...
public class AirlinesAdapter(IBus _bus) : IEventsAdapter
{
    ...
    public async Task Subscribe<T>(string subscriptionId, EventHandler<T>? handler, CancellationToken token = default)
    {
        if (handler == null)
        {
            handler = new EventHandler<T>(DefaultHandleTextMessage);
        }
